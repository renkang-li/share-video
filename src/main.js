...
app.innerHTML = `
  <main class="page-shell">
    ...
    <button id="scrollTopButton" class="scroll-top-btn is-hidden" type="button" aria-label="回到顶部" title="回到顶部">
...
// Scroll to top behavior & listener
function handleScrollVisibility() {
  const windowScrolled = window.scrollY > 150
  const listScrolled = refs.videoList.scrollTop > 150
  if (windowScrolled || listScrolled) {
    refs.scrollTopButton.classList.remove('is-hidden')
  } else {
    refs.scrollTopButton.classList.add('is-hidden')
  }
}

window.addEventListener('scroll', handleScrollVisibility, { passive: true })
refs.videoList.addEventListener('scroll', handleScrollVisibility, { passive: true })

refs.scrollTopButton.addEventListener('click', () => {
  window.scrollTo({ top: 0, behavior: 'smooth' })
  refs.videoList.scrollTo({ top: 0, behavior: 'smooth' })
})
